...
let parse_file filepath =
	...
	let _ = Unix.create_process
	 "camlp4" [| "camlp4"; "-parser"; "Ocaml"; "-printer"; "d"; filepath |] 
...
(*				"camlp4" [| "camlp4"; "-parser"; "Ocaml"; "-printer"; "Camlp4AstDumper"; temp_file_name |] *)
              "camlp4" [| "camlp4"; "-parser"; "Ocaml"; "-printer"; "d"; temp_file_name |] 
				Unix.stdin camlp4_stdout Unix.stderr in
		Unix.close camlp4_stdout;
...
let cmd_print_moduList () = 
    let filepath = read_line() in
    ASTParse.parse_ast_in_xml (parse_file filepath)
;;
...
let _ =
	...
	(** Pierre's having fun
      print_endline "Choose between : ";
	    print_endline "end | exit | quit | print | astFromFile | astFromInput | ast "; **)
	print_endline "ok";
	print_eot ();
