...
and print_ctyp f = function (* Representation of types                                     *)
    ...
    (** Package type **)
    | TyPkg(loc, module_type1) -> pp f "<TyPkg loc='%s'><module_type1>%a</module_type1></TyPkg>" (string_of_loc loc) print_module_type module_type1
    (* $s$ *) (** Antiquotation *)
...
and print_expr f = function (* The type of expressions                                    *)
    ...
    (* fun *) (** Function without match case *)
    | ExFUN(loc, name, expr1) -> pp f "<ExFUN loc='%s'><name>%s</name><expr>%a</expr></ExFUN>" (string_of_loc loc) (escape_string name) print_expr expr1
    ...
    | ExOpI(loc, ident1, expr1) -> pp f "<ExOpI loc='%s'><ident>%a</ident><expr1>%a</expr1></ExOpI>" (string_of_loc loc) print_ident ident1 print_expr expr1
    (** expression to pack a module as a first-class value **)
    | ExPkg(loc, module_expr1) -> pp f "<ExPkg loc='%s'><module_expr>%a</module_expr></ExPkg>" (string_of_loc loc) print_module_expr module_expr1
...
and print_module_expr f = function (* The type of module expressions                             *)
    ...
    (** module expression to unpack a first-class value as a module**)
    | MePkg(loc,expr1) -> pp f "<MePkg loc='%s'><expr>%a</expr>/MeAnt>" (string_of_loc loc) print_expr expr1
...
and print_class_sig_item f = function (* The type of class signature items                          *)
    ...
    | CgVal(loc, name, mutable_flag, virtual_flag, ctyp1) -> pp f "<CgVal loc='%s'><name>%s</name><is_virtual>%a</is_virtual><is_mutable>%a</is_mutable><ctyp>%a</ctyp></CgVal>" (string_of_loc loc) (escape_string name) print_mutable_flag mutable_flag print_virtual_flag virtual_flag print_ctyp ctyp1
...
and print_class_str_item f = function (* The type of class structure items                          *)
    ...
    | CrInh(loc, override_flag, class_expr1, name) -> pp f "<CrInh loc='%s'><isOverridable>%a</isOverridable><class_expr>%a</class_expr><name>%s</name></CrInh>" (string_of_loc loc) print_override_flag override_flag print_class_expr class_expr1 (escape_string name)
    (* initializer e *)
    | CrIni(loc, expr1) -> pp f "<CrIni loc='%s'><expr>%a</expr></CrIni>" (string_of_loc loc) print_expr expr1
    (* method (private)? s : t = e or method (private)? s = e *)
    | CrMth(loc, name, override_flag, private_flag, expr1, ctyp1) -> pp f "<CrMth loc='%s'><name>%s</name><isOverridable>%a</isOverridable><is_private>%a</is_private><expr>%a</expr><ctyp>%a</ctyp></CrMth>" (string_of_loc loc) (escape_string name) print_override_flag override_flag print_private_flag private_flag print_expr expr1 print_ctyp ctyp1
    (* value (mutable)? s = e *)
    | CrVal(loc, name, override_flag, mutable_flag, expr1) -> pp f "<CrVal loc='%s'><name>%s</name><isOverridable>%a</isOverridable><is_mutable>%a</is_mutable><expr>%a</expr></CrVal>" (string_of_loc loc) (escape_string name) print_override_flag override_flag print_mutable_flag mutable_flag print_expr expr1
    (* method virtual (private)? s : t *)
